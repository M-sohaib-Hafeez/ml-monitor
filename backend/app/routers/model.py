...
def evaluate_classification(req: ClassificationMetricsRequest):
    ...
    return {
        ...
        "model_health": health,
        "total_samples": len(req.y_true),
        "correct_predictions": int((y_true == y_pred).sum()),
        "recommendation": (
            "Model performing excellently" if accuracy > 0.9
            else "Consider fine-tuning" if accuracy > 0.75
            else "Retraining strongly recommended"
        ),
    }

@router.post("/evaluate/regression")
def evaluate_regression(req: RegressionMetricsRequest):
    if len(req.y_true) != len(req.y_pred):
        raise HTTPException(status_code=400, detail="y_true and y_pred must have same length")
    if len(req.y_true) < 2:
        raise HTTPException(status_code=400, detail="Need at least 2 samples")
    ...
    return {
        ...
        "total_samples": len(req.y_true),
        "recommendation": (
            "Model performing excellently" if r2 > 0.9
            else "Consider fine-tuning" if r2 > 0.75
            else "Retraining strongly recommended"
        ),
    }
